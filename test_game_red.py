...
def CapitalLetters(a,LenOfStr): # Расставление заглавных букв и переносов в строках с информвцией
        ...
        for z in range(1,len(tk)):
            if tk[z]==' ' and tk[z+1]!='и':
                aa+=tk[z]
                aa+=tk[z+1].upper()
            elif tk[z]==' ' and tk[z+1]=='и':
                aa+=tk[z:z+2]
            elif tk[z-1]!=' ':
                aa+=tk[z]
        tt=''
    ...
    if (tmp!=()):
        ...
        for i in range(13):
            ...
            if type(mass[op][i]) is str:
                ...
                if i==9:
                    print(er[1])
                MassOfPrints[i]=Label(panelFrame2,text=MassOfBeginings[i]+er[0],width=35,height=2+er[1]//2, bg = 'white').place(x=0,y=a)
            else:
                MassOfPrints[i]=Label(panelFrame2,text=MassOfBeginings[i]+str(arr[i])+TinyMass[i],width=35,height=2, bg = 'white').place(x=0,y=a)
            a+=32
        try:
...
def tst(event): #Основная функция для теста
    ...
    TotalNumberOfQuests=Label(panelFrame, height=2,text='Всего отвечено: '+str(AmountOfAskedQuests)+' вопросов', width=22, bg='white',fg='black')
    TotalNumberOfQuests.place(x=89,y=4)

    AnsweredRight=Label(panelFrame, height=2,text='Отвечено правильно: '+str(AmountOfAnsweredRight)+' вопросов', width=34, bg='white',fg='black')
